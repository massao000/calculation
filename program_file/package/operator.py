...
class Writing:
    ...
    def __init__(self, level_display, issue, problem, time_attack, correct_answer_rate, fraction):
    ...
    def show(self):
        ...
        for i, issue in enumerate(self.issue_list, 1):
            print(f'{"{0:02d}".format(i)}: {issue}')
...
class Operator:
    ...
    def end_time_show(self):
        ...
        Writing(self.level, self.mode, self.random_problem_number, self.time_attack, self.correct_answer_count, self.fraction).show()
        self.text_file()
    ...
    def text_file(self):
        ...
        if os.path.exists(f'{app_exists_path}\\{Writing.NEW_FILE_NAME}\\{txt}'):
            with open(f'{app_exists_path}\\{Writing.NEW_FILE_NAME}\\{txt}', 'a', encoding='UTF-8')as f:
                ...
                for i, problem_writing in enumerate(self.mode, 1):
                    f.write(f'\n{"{0:02d}".format(i)}：{problem_writing}')
    ...
    def answer_data(self):
        if self.answer == self.operation_answer:
            ...
            you_answer = (f'{self.issue}{str(self.answer)} {Writing.CORRECT_ANSWER}')
            self.mode.append(you_answer)
            ...
            you_answer = (f'{self.issue}{str(self.answer)} {Writing.INCORRECT_ANSWER}')
            self.mode.append(you_answer)
            self.csv_data.append(you_answer)
